...
def verify_apk(apk_path: Path, is_release: bool = False, expected_sha256: str | None = None) -> bool:
    if not apk_path.exists():
        print(f"❌ APK not found: {apk_path}")
        return False

    print(f"🔍 Inspecting APK: {apk_path} ({apk_path.stat().st_size / (1024*1024):.2f} MB)")

    zipalign_bin = find_tool("zipalign")
    apksigner_bin = find_tool("apksigner")

    # 1. Verify 4-byte alignment
...
if __name__ == "__main__":
    ...
    ok = verify_apk(apk_file, is_release=is_rel, expected_sha256=expected_hash)
    if not ok:
        sys.exit(1)
