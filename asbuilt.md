...
- **`Haven`**: Hardware-backed consent gate, 3-tier action permissions, and durable persistent audit journal.
...
- 305 verified unit test cases covering authentication, leasing, and channel management.
...
- 232 verified unit test cases covering stream framing and client state machine.
...
### `:core:core-mcp`
- Embedded HTTP/SSE JSON-RPC 2.0 daemon running on port 8070 with dynamically generated bearer tokens.
...
- **ProfileRepository**: Durable profile storage with encoded key persistence initialized on application boot.
...
- **DynamicAccessibilityBridge + McpAccessibilityService**: Real Android AccessibilityService dynamic DI wiring for UI hierarchy dumps, coordinate taps, swipes, and app launches.
