...
* **Verification**: Manually dispatched `ci.yml` (the trigger that forces the full-history scan) — confirmed the `.gitleaks.toml`-only attempt still failed with the same fingerprint. Re-dispatched after switching to `.gitleaksignore`; see this PR's CI run for the actual result rather than taking it on faith.
